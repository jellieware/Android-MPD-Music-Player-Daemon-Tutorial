...
ffmpeg -hide_banner -loglevel error -f s16le -ar 44100 -ac 2 -i ~/mpd.fifo \
       ...
       -map "[out]" -f wav - | \
ffplay -nodisp -af "aresample,aformat=channel_layouts=stereo,equalizer=f=100:width_type=h:width=50:g=20,bs2b=fcut=650:feed=9.5,volume=5,sofalizer=sofa='/storage/emulated/0/ClubFritz1.sofa'" -fflags nobuffer -flags low_delay -i - >/dev/null 2>&1 &
#!/bin/bash
